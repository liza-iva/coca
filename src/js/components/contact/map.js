export const useMap = () => {
  ...
  async function main() {
    ...
    // Координаты центра карты
    const CENTER_COORDINATES = [120.638275176010463, 19.29606083828731];
    ...
    map = new YMap(document.getElementById('map'), {
      location: LOCATION,
      behaviors: ['default'], // Можно оставить стандартные поведения
      controls: [], // Указываем, что контролы не нужны, если не хотите добавлять
    });
    ...
    // const controlsTopRight = new YMapControls({ position: 'top right' });
    // controlsTopRight.addChild(new YMapGeolocationControl());

    // map.addChild(controlsRight);
    // map.addChild(controlsTopRight);
    ...
    balloon.className = 'custom-balloon';
    balloon.style.position = 'absolute';
    balloon.style.display = 'none'; // Скрываем балун по умолчанию
    balloon.innerHTML = `
  ...
  `;
    ...
    // Функция для открытия балуна
    function openBalloon(coords) {
      balloon.style.display = 'block';
      balloon.style.left = `${coords[0]}px`; // Устанавливаем координаты
      balloon.style.top = `${coords[1]}px`;
    }
    ...
    imgContainer.onclick = (event) => {
      event.stopPropagation();

      // Получаем положение контейнера маркера на экране
  const rect = imgContainer.getBoundingClientRect();

  // Устанавливаем позицию балуна немного выше маркера
  openBalloon([(rect.left - 130) + rect.width / 2, rect.top]); // Центрируем по ширине и смещаем вверх
    };
    ...
    function adjustCenter() {
      ...
      let newCenter;
      if (screenWidth >= 769) {
        newCenter = [27.638275176010463, 19.29606083828731];
      } else if (screenWidth >= 577) {
        newCenter = [50.638275176010463, 19.29606083828731];
      } else {
        newCenter = CENTER_COORDINATES; // Значение по умолчанию
      }

      map.update({ location: { center: newCenter } });
    }
